...
export class ChatService {
  ...
  constructor(private httpClient: HttpClient
    ...
    this.sock = io('http://10.13.2.7:3000/chat', {
      withCredentials: true,
    });
    this.sock.on(
      'recMessage', (data) => {
  ...
  getThisChat(id: number) {
    return this.httpClient.get(`http://10.13.2.7:3000/chat/find/` + id, { withCredentials: true, })

  }

  getThisChatMsgs(id: number) {
    return this.httpClient.get(`http://10.13.2.7:3000/message/for` + id, { withCredentials: true, })

  }

  getChatrooms() {
    return this.httpClient.get(`http://10.13.2.7:3000/chat/user`, { withCredentials: true, })
  }

  getChatDM() {
    return this.httpClient.get(`http://10.13.2.7:3000/chat/DM`, { withCredentials: true, })
  }
  joinChatroom(chat: createChatroom) {
    return this.httpClient.post(`http://10.13.2.7:3000/chat/create`, chat, { withCredentials: true, });
  }
  ...
  addUser(user: addUserDTO) {
    return this.httpClient.post<addUserDTO>(`http://10.13.2.7:3000/user`, user, { withCredentials: true, });
  }
  getUser() {
    return this.httpClient.get("http://10.13.2.7:3000/user", { withCredentials: true, })
  }
  addUserToChatRoom(id: string, user: string, password: string) {
    ...
    return this.httpClient.put<string>(`http://10.13.2.7:3000/chat/${id}/add/member`, dto, { withCredentials: true, });
  }
  addAdminToChatRoom(id: string, admin: string, user: string) {
    ...
    console.log("dto", dto, "id == ", id);
    return this.httpClient.put<string>(`http://10.13.2.7:3000/chat/${id}/add/admin`, dto, { withCredentials: true, });
  }
