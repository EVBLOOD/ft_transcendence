...
export class MatchService {
  constructor(
    ...
    private readonly StatasticsRepo: Repository<Statastics>,
  ) {}
  ...
  async getLeadering() {
    return (
      ...
      return {
        user: { username: info.User.username, avatar: info.User.avatar },
        matchPlayed: info.total,
        win: info.win,
        ratio: info.score,
      };
    });
  ...
  async getWinnCalc(id1: number, id2: number) {
    return {
      number: await this.matchRepository
        ...
        .leftJoinAndSelect('match.winner', 'winner')
        .where(
          'winner.id = :id1 AND (player1.id = :id2 OR player2.id = :id2)',
          { id1: id1, id2: id2 },
        )
        .getCount(),
    };
