...
export class FriendshipGateway implements OnGatewayInit {
  ...
  private connections: Map<number, Set<Socket>> = new Map<
    number,
    Set<Socket>
  >();
  ...
  async handleConnection(client: Socket, ...args: any[]) {
    ...
    if (!userSockets) userSockets = new Set<Socket>();
  ...
  async handleFriendRequest(client: any, payload: number) {
    ...
    let replay: any;
    try {
      replay = await this.friendshipService.create(xyz.sub, payload);
  ...
  async handleAcceptFriendRequest(client: any, payload: number) {
    ...
    let replay: any;
    try {
      replay = await this.friendshipService.accepting(xyz.sub, payload);
      if (!replay) return 'UserNotFound';
    } catch (err) {
      return 'UserNotFound';
    }
  ...
  async handleBlockUser(client: any, payload: number) {
    ...
    let replay: any;
    try {
      replay = await this.friendshipService.blocking(xyz.sub, payload);
      if (!replay) return 'UserNotFound';
    } catch (err) {
      return 'UserNotFound';
    }
  ...
  async handleUnblockUser(client: any, payload: number) {
    ...
    let replay: any;
    try {
      replay = await this.friendshipService.unblock(xyz.sub, payload);
      if (!replay) return 'UserNotFound';
    } catch (err) {
      return 'UserNotFound';
    }
  ...
  async handleDeleteFriendship(client: any, payload: number) {
    ...
    let replay: any;
    try {
      replay = await this.friendshipService.remove(xyz.sub, payload);
      if (!replay) return 'UserNotFound';
    } catch (err) {
      return 'UserNotFound';
    }
