...
export class FriendshipService {
  ...
  async blocklist(id: number, skip: number, take: number) {
    ...
    blocklist.forEach((elmnt) => {
      if (elmnt.sender == id) elmnt.sender = elmnt.receiver;
    });
    return blocklist;
  }
  ...
  async blocking(id: number, userTo_block: number) {
    ...
    if (friendship[0].sender == id) friendship[0].blocked_by = 'sender';
    else friendship[0].blocked_by = 'receiver';
