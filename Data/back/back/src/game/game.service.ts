...
export class GameService {
  ...
  constructor(
    ...
    @InjectRepository(Statastics)
    private readonly StatisticsRepo: Repository<Statastics>,
  ) {
    setInterval(() => {
      ...
      Object.entries(this.activeGameInstances).forEach(([key, value]) => {
        if (value.inactive) {
          ...
          this.matchService.create({
            ...
            player2Id,
            winnerId:
              value.score.player1 > value.score.player2 ? player1Id : player2Id,
          });
          this.StatisticsRepo.createQueryBuilder()
            .update()
            .set({
              total: () => 'total + 1',
              score: () =>
                `score + (${
                  value.score.player1 > value.score.player2 ? 3 : -1
                })`,
              win: () =>
                `win + (${value.score.player1 > value.score.player2 ? 1 : 0})`,
            })
          ...
          this.StatisticsRepo.createQueryBuilder()
            ...
            .set({
              ...
              score: () =>
                `score + (${
                  value.score.player2 > value.score.player1 ? 3 : -1
                })`,
              win: () =>
                `win + (${value.score.player2 > value.score.player1 ? 1 : 0})`,
            })
      ...
      if (this.queue.length >= 2) {
        let [player1, player2]: { id: number; socket: Socket }[] = sampleSize(
          this.queue,
          2,
        );
        if (
          this.onlineUsers.get(player1.id).has(player1.socket) &&
          this.onlineUsers.get(player2.id).has(player2.socket)
        ) {
          ...
          this.activeGameInstances[`${player1.id},${player2.id}`] =
            new GameInstance(player1.socket, player2.socket);
          ...
          this.queue = this.queue.filter((elemets) => {
            return elemets.id != player1.id;
          });
        else if (!this.onlineUsers.get(player1.id).has(player1.socket))
          this.queue = this.queue.filter((elemets) => {
            return elemets.id != player2.id;
          });
      }
        ...
        invitedUser.forEach((socket_) => {
          ...
          socket_.once('inviteResponse', (response) => {
            invitedUser.forEach((socket__) => {
              if (
                socket__.id == socket_.id &&
                response == true &&
                !this.currentPlayers.find((player) => player.id === id2) &&
                !this.currentPlayers.find((player) => player.id === id1) &&
                this.onlineUsers.get(id1)?.has(socket)
              ) {
                if (this.queue.find((player) => player.id === id1))
                  this.queue = this.queue.filter((elemets) => {
                    return elemets.id != id1;
                  });
                if (this.queue.find((player) => player.id === id2))
                  this.queue = this.queue.filter((elemets) => {
                    return elemets.id != id2;
                  });
                ...
                this.activeGameInstances[`${id1},${id2}`] = new GameInstance(
                  socket,
                  socket__,
                );
                this.currentPlayers.push({ id: id2, socket: socket__ });
