...
export class GameInstance {
  ...
  constructor(private player1: Socket, private player2: Socket) {
    ...
    Body.setVelocity(
      this.ball,
      this.getNewStart(GAMEWIDTH, GAMEHEIGHT).velocity,
    );
    ...
    player1.on('sendMyPaddleState', (state) => {
      ...
      const offset = PositionState.createPositionState(
        builder,
    ...
    player1.on('disconnect', () => {
      player2.emit(
        'changeState',
        JSON.stringify({ gameState: 'Finished', isWin: true }),
      );
    ...
    setTimeout(() => {
      player1.emit(
        ...
        JSON.stringify({
          gameState: 'Playing',
          playerNumber: 1,
          color: Common.choose([Color.White, Color.Blue, Color.Green]),
        }),
      );
      player2.emit(
        'changeState',
  ...
  private setPlayerWon(player: PlayerNumber) {
    if (player === PlayerNumber.One) {
      ...
      this.player2.emit(
        'changeState',
        JSON.stringify({ gameState: 'Finished', isWin: false }),
      );
    } else {
      this.player1.emit(
        'changeState',
    ...
    Events.on(this.engine, 'afterUpdate', () => {
      ...
      if (
        (this.ball.velocity.y > 0 &&
          this.ball.position.y + BALLRADIUS >= GAMEHEIGHT - 5) ||
  ...
  private sendBallPosition() {
    const builder = new flatbuffers.Builder();
    const ballStateOffset = PositionState.createPositionState(
      builder,
  ...
  private getNewStart(gameWidth: number, gameHeight: number) {
    ...
    const angle = Common.random(
      Common.choose([-1, 1]) * 25,
      Common.choose([-1, 1]) * 55,
    );
