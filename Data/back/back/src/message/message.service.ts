...
export class MessageService {
	...
	async getMessageByUserId(ID: number) {
		const message = await this.messageRepo.find({
			order: { // get messages in ascending order
				id: 'asc',
			},
			relations: { // load the userId and chatroom entity in a singel query
				userId: true,
				chatRoomId: true,
			},
			where: { // only care bout the with the passed ID
				userId: {
					id: ID,
				},
			},
			select: { // we only care about these values
				userId: {
					id: true,
					userName: true,
				},
